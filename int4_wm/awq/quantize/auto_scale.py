...
def auto_scale_block(module, module_kwargs,
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        # w_max: [2560]; weight: [7680, 2560] --> kvq
        ...
        with torch.no_grad():
            ...
            if isinstance(org_out, tuple):
                org_out = org_out[0]
        # x: [60, 512, 2560]; org_out: [60, 512, 2560]; x_max: [2560]
        x_max = get_act_scale(x)
        x_feat = torch.max(x_max)/x_max
        #import pdb; pdb.set_trace()
        ...
        for ratio in range(n_grid):
            ...
            scales = scales / (scales.max() * scales.min()).sqrt() # scale: [2560]
            #import pdb; pdb.set_trace()
        ...
        return best_scales.detach(), x_feat
    ...
    def _auto_get_scale(prev_op, layers, inp, module2inspect=None, kwargs={}):
        ...
        #import pdb; pdb.set_trace()
        scales, x_feat = _search_module_scale(module2inspect, layers, inp, kwargs)
        scales = scales.detach().cpu()
        x_feat = x_feat.detach().cpu()
        # prev_op_name, [layer_name], scale
        return (get_op_name(module, prev_op), tuple([get_op_name(module, m) for m in layers]), scales), (get_op_name(module, prev_op), tuple([get_op_name(module, m) for m in layers]), x_feat)

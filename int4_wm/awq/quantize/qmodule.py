...
class WQLinear(nn.Module):
    def __init__(self, w_bit, group_size, in_features, out_features, bias, dev):
        ...
        self.register_buffer('qweight', torch.zeros((out_features, in_features // self.pack_num), dtype=torch.int32, device=dev))
        #self.register_buffer('intweight', torch.zeros((out_features, in_features), dtype=torch.int32, device=dev))
        ...
        self.register_buffer('scales', torch.zeros((out_features, calculate_zeros_width(in_features, self.group_size) * self.pack_num), dtype=torch.float16, device=dev))
        
        if bias:
            self.register_buffer('bias', torch.zeros((out_features), dtype=torch.float16, device=dev))
        else:
            self.bias = None
        #self.intweight = torch.zeros((out_features, in_features), dtype=torch.int32, device=dev)
    ...
    def from_linear(cls, linear, w_bit, group_size, init_only=False, scales=None, zeros=None):
        ...
        #print("intweight", intweight.size(), linear.out_features, linear.in_features)
    ...
    def int_to_qweight(self, intweight):
        qweight = torch.zeros((intweight.shape[0], intweight.shape[1] // 32 * self.w_bit), dtype=torch.int32, device=intweight.device)           
        #import pdb; pdb.set_trace()
        #print("intweight", intweight.size(), linear.out_features, linear.in_features)
        for col in range(intweight.shape[1] // self.pack_num):
            if self.w_bit == 4:
                # order_map = [0, 2, 4, 6, 1, 3, 5, 7]
                order_map = [0, 1, 2, 3, 4, 5, 6, 7]
            else:
            ...
            for i in range(self.pack_num):
                qweight_col = intweight[:, col * self.pack_num + order_map[i]]
                qweight[:, col] |= qweight_col << (i * self.w_bit)
        self.qweight = qweight
        return qweight

    def qweight_to_intweight(self, qweight):
        intweight = torch.zeros((qweight.shape[0], self.in_features), dtype=torch.int32, device=qweight.device)

        for col in range(intweight.shape[1] // self.pack_num):
            for i in range(self.pack_num):
                qweight_col = (qweight[:, col] >> (i * self.w_bit)) & ((1 << self.w_bit) - 1)
                intweight[:, col * self.pack_num + i] = qweight_col
